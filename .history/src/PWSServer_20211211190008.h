...
{
public:
  void begin(AsyncWebServer* ws, Preferences* c, Adafruit_BME280* sensor);
  bool wifiConnect();
  bool mqttConnect();
  bool bmeConnect();
  IPAddress getIPAddr();
...
private:
  ...
  void setupServer()
  {
    ...
    webServer->on("/api/status", HTTP_GET, [this](AsyncWebServerRequest *request) {
      ...
      json += "\"wifiFailed\": " + String(!isWifiConnected()) + ",";
      json += "\"mqttFailed\": " + String(!isMqttConnected()) + ",";
      json += "\"hasWifiConfig\": " + String(hasWifiConfig()) + ",";
      json += "\"hasMqttConfig\": " + String(hasMqttConfig()) + ",";
      json += "\"bmeConnected\": " + String(isBmeConnected()) + "";
      json += "}";
