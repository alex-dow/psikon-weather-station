...
void get_spiff_data(char* val, const char* id, const char* defaultValue) {
  char fn[256];
  sprintf(fn, "/%s", id);
...
/*
  const char* mqtt_host = get_spiff_data(mqtt_host, "mqtt_host", "localhost");
  const char* mqtt_port = get_spiff_data(mqtt_port, "mqtt_port", "1883");
  const char* mqtt_username = get_spiff_data(mqtt_username, "mqtt_username", "");
  const char* mqtt_password = get_spiff_data(matt_password, "mqtt_password", "");
  */
