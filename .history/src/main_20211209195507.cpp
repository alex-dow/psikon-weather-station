...
void setup() {
  ...
  const char* whoami = get_spiff_data("me", "0");
  char me[256];

  if (strcmp(whoami, "0") == 0) {
    sprintf(me, "psikon-weather-%d", rand() % 99999);
  } else {
    strcpy(me, whoami);
  }
  ...
  WiFiManagerParameter _me("me", "Name", "", 128);
  ...
  if (saveConfig) {
    ...
    set_spiff_data("me", _me.getValue());
    Log.infoln("done");
...
*/
  ...
  const char* me = get_spiff_data("me", "");

  sprintf(topic, "/psikon/weather/%s", me);
  ...
  if (client.connect(me, mqtt_username, mqtt_password)) {
