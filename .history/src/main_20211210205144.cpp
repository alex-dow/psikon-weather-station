...
class CaptivePortalHandler : public AsyncWebHandler {
  ...
  void handleRequest(AsyncWebServerRequest *request) {
    Log.infoln("Portal url: %s", request->url().c_str());
    request->send_P(SPIFFS, "/index.html", "text/html", false);
...
void setup() {
  ...
  webServer.on("/global.css", HTTP_GET, [] (AsyncWebServerRequest *request) {
    request->send(SPIFFS, "/global.css", String(), false);
  });

  webServer.on("/bundle.css", HTTP_GET, [] (AsyncWebServerRequest *request) {
    request->send(SPIFFS, "/bundle.css", String(), false);
  });

  webServer.on("/bundle.js", HTTP_GET, [] (AsyncWebServerRequest *request) {
    request->send(SPIFFS, "/bundle.js", String(), false);
  });
