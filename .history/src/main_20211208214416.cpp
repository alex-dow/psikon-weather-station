...
void setup() {
  ...
  #ifdef RESET_ENABLED
  Log.warningln("Formatting all data");
  SPIFFS.format();
  wifiManager.resetSettings();
  #endif

  Log.infoln("Loading MQTT configuration");

  char* mqtt_host = get_mqtt_host();
  uint16_t mqtt_port = get_mqtt_port();
  char* mqtt_username = get_mqtt_username();
  char* mqtt_password = get_mqtt_password();
  char mqtt_port_str[5];
  sprintf(mqtt_port_str, "%d", mqtt_port);

  Log.info("MQTT URL: mqtt://%s:%s@%s:%d", mqtt_host, mqtt_port, mqtt_username, mqtt_password);)


  #ifdef RESET_ENABLED
  Log.warningln("Formatting all wifi data");
  wifiManager.resetSettings();
  #endif
